...
const SPOILAGE_STATES = {
  fresh: { maxHours: 24, color: '#10b981', emoji: '🟢', label: 'Fresh' },
  aging: { maxHours: 48, color: '#f59e0b', emoji: '🟡', label: 'Aging' },
  urgent: { maxHours: 72, color: '#ef4444', emoji: '🔴', label: 'Urgent' },
  spoiled: { color: '#6b7280', emoji: '⚫', label: 'Spoiled' },
};
...
export default function SushiTrader() {
  ...
  const updateGameTime = () => {
    setGame(prev => {
      ...
      const elapsedHours = Math.floor(elapsedMs / 3600000); // 1 real hour per game hour
      ...
      let newGame = {
        ...
        day: Math.floor((prev.totalHours + elapsedHours) / 24) + 1,
        lastActionTime: now,
      };
      ...
      if (newGame.day > prev.day) {
        const eateryRevenue = newGame.restaurants.reduce((s, r) => {
          const tier = RESTAURANTS.find(t => t.id === r.tierId);
          return s + (tier ? tier.dailyRevenue : 0);
        }, 0);
  ...
  const completeTravel = () => {
    setGame(prev => {
      ...
      const newDay = Math.floor(arrivalHour / 24) + 1;
  ...
  const getDailyEateryRevenue = () => game.restaurants.reduce((s, r) => {
    const tier = RESTAURANTS.find(t => t.id === r.tierId);
    return s + (tier ? tier.dailyRevenue : 0);
  }, 0);
              ...
              <View style={styles.cityTopRow}>
                <View style={styles.cityLeft}>
                  ...
                  <View style={styles.cityNameBlock}>
                    ...
                    <Text style={styles.cityMetaText}>Day {game.day} · Hour {game.totalHours % 24}</Text>
                  </View>
                </View>
                {currentEvent ? (
